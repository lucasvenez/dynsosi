	...
	function parser($document,$is_file=TRUE){ 
    	...
    	if($is_file){ 
      		if(file_exists($document)){          
        		...
        		print "<b><h3>O arquivo <font color=\"blue\">$document</font> n&atilde;o existe.</b><h3>";
	    	} 
		}else{ 
      		$this->texto = $document;     
    	} 
    	...
    	if(isset($ext_val)){ 
     	...
     	if (!xml_parse($parser, $this->texto,TRUE)) { 
	        ...
	        $this->get_err = "<font color=\"black\" size=\"3\" face=\"arial\">Fonte do documento: $document<br/>". 
                         "<b>XML erro: </b>".xml_error_string(xml_get_error_code($parser))." <b>at</b> linha ". 
                         xml_get_current_line_number($parser)." e coluna $err_col<br/><b> Texto: </b>$text1$text2$text3";
    	    $result = FALSE;
	    } 
